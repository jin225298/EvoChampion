...
from config.settings import DOMAIN, get_session_dir, get_classifier_labels
...
def _fallback_prompt_design(goal: str) -> dict:
    ...
    if DOMAIN == "code":
        return {
            "domain_goal": goal_text or "write python functions passing tests",
            "target_capabilities": [
            ...
            "search_keywords": [
                ...
                "humaneval style code",
            ],
            "boundary_signals": [
                "tests missing",
                "syntax or compile errors",
                "assertion failures on edge cases",
            ],
            "rare_signals": [
                "recursive functions",
                "string manipulation",
            ...
            "classifier_labels": [
                "arithmetic", "string", "list", "logic", "recursion", "unknown",
            ],
            "classifier_label_notes": {},
            "instruction_prefix": "Write a Python function that solves the following problem. Output only the code, no explanation.\n",
        }
    return {
...
def _apply_leaf_design_decisions(state: EvoState, fallback: dict, round_id: int) -> tuple[dict, list[str]]:
    ...
    raw_goal, ok = decide_json_leaf(
        ...
        prompt=get_agent_prompt("prompt_designer.domain_goal", PROMPT_DESIGNER_DOMAIN_GOAL_PROMPT),
    ...
    raw_caps, ok = decide_json_leaf(
        ...
        prompt=get_agent_prompt("prompt_designer.capabilities", PROMPT_DESIGNER_CAPABILITIES_PROMPT),
    ...
    raw_kw, ok = decide_json_leaf(
        ...
        prompt=get_agent_prompt("prompt_designer.keywords", PROMPT_DESIGNER_KEYWORDS_PROMPT),
    ...
    raw_b, ok = decide_json_leaf(
        ...
        prompt=get_agent_prompt("prompt_designer.boundary", PROMPT_DESIGNER_BOUNDARY_PROMPT),
    ...
    raw_r, ok = decide_json_leaf(
        ...
        prompt=get_agent_prompt("prompt_designer.rare", PROMPT_DESIGNER_RARE_PROMPT),
    ...
    raw_labels, ok = decide_json_leaf(
        ...
        prompt=get_agent_prompt("prompt_designer.labels", PROMPT_DESIGNER_LABELS_PROMPT),
...
def _compose_prompt_pack(design: dict) -> dict:
    pack = {
        ...
        "prompts": build_agent_prompt_pack(DOMAIN),
    }
    # 组装要注入的 goal 信息文本块
    target_line = (
        "\n\n本轮训练目标槽位:\n"
